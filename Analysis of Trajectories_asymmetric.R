Gillespie_fct_deme <- function(Nit, fA, gA, NA10, NA20, fB, gB, NB10, NB20, K1, K2, mu, m12, m21, theta1, theta2, tlist) {
  ...
  for (i in 1:Nit) { # Loop over the number of replicates
    ...
    while ((NA1 + NA2 + NB1 + NB2 != 0) && t < max(tlist)) { # running untill tmax reached or no individuals left
      ...
      # update populations size according to chosen reaction
      if (ir2 == 1) {
...
m12 <- 0.008333     # m12: migration rate from deme 1 to deme 2
m21 <- 0.001667      # m21: migration rate from deme 2 to deme 1
...
for (i in 1:Iter) {
  # Call function Gillespie_fct_deme with appropriate arguments
  result <- Gillespie_fct_deme(Nit, fA, gA, NA10, NA20, fB, gB, NB10, NB20, K1, K2, mu, m12, m21, theta1, theta2, tlist)
  ...
  plot <- ggplot(df_all, aes(x = effective_time, y = value, color = Population, group = replicate)) +
    ...
    geom_line(size = 0.5) +
    ggtitle("Population trajectory asymmetric, beta 5, Tmig 1e0") +
    annotate("text", x = 750, y = 140, label = "deme 1 deteriorated", color = "black", size = 4.5) +
    annotate("text", x = 1250, y = 140, label = "deme 1 & 2 deteriorated", color = "black", size = 4.5)
  
  # Save plot as image with increasing number
  ggsave(paste0("D:/FS24/BSc/Work in Progress/Plotting trajectories/Analysis of Trajectories/Deme Dependent/beta 5 _ Tmig 1e0/plot_", i, ".jpg"), plot, width = 10, height = 6)
}
