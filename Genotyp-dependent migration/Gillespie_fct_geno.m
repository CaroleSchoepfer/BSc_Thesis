function rescue_status = Gillespie_fct_geno(Nit, fA, gA, NA10, NA20, fB, gB, NB10, NB20, K1, K2, mu, mWT, mM, theta1, theta2)
  ...
  for i = 1:Nit
    ...
    while cumul(ir2) < r2 * T % screening the sampling tower
        ir2 = ir2 + 1; 
    end
        ...
        if final_population(2) + final_population(4) > 0  % if at the last timepoint of the simulation there are still Mutant individuals left
            rescue_status(i) = 1;                           % we consider the population as rescued
        else                                              % if no individuals left
            rescue_status(i) = 0;                           % we consider the population as not rescued
        end
  end
    
  if Nit > 1  % this part is handy for the automWTted analysis. In any case we will have only number as an output
